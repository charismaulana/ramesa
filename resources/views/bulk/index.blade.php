...
@section('content')
    ...
    <form action="{{ route('bulk.store') }}" method="POST" id="bulk-form" enctype="multipart/form-data">
                ...
                <div class="col-2">
                    <div class="form-group">
                        ...
                        <input type="date" name="date" id="date" class="form-control"
                            value="{{ old('date', date('Y-m-d')) }}" requiredonchange="autoSelectProof()">
                    </div>
                </div>
                <div class="col-2">
                    <div class="form-group">
                ...
                <div class="col-2">
                    <div class="form-group">
                        ...
                        <select name="location" id="location" class="form-control" required
                            onchange="autoSelectProof(); reorderGroups();">
                                ...
                                <select name="existing_proof" id="existing_proof" class="form-control" style="flex: 1;"
                                    ...
                                    @foreach($recentProofs as $proof)
                                        <option value="{{ $proof['path'] }}" data-url="{{ $proof['url'] }}"
                                            data-date="{{ $proof['date'] }}" data-location="{{ $proof['location'] }}">
                                            {{ $proof['filename'] }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <!-- Group Upload Status Panel -->
        @if($groups->count() > 0)
            <div class="card" id="groupStatusCard" style="display: none;">
                <div class="card-header d-flex justify-between align-items-center">
                    <h2 class="card-title">📋 Group Upload Status</h2>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="refreshGroupStatus()">
                        <i class="bi bi-arrow-clockwise"></i> Refresh
                    </button>
                </div>
                <div id="groupStatusContent" style="padding: 1rem;">
                    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;" id="groupStatusBadges">
            ...
            <div style="padding: 1rem; border-top: 1px solid var(--card-border);">
                ...
                <div id="mealTotalsContainer"
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: center;">
                        ...
                        <span class="badge badge-warning" style="font-size: 0.85rem;">🍪 Snack: <strong
                                id="totalSnack">0</strong></span>
            ...
            <form id="visitorForm" onsubmit="addVisitors(event)">
                ...
                <div class="form-group">
                    ...
                    <div class="meal-checkboxes" style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                        ...
                        <label class="meal-checkbox">
                            <input type="checkbox" id="visitorDinner" value="dinner" onchange="handleVisitorDinnerChange()">
                            <span>🌙 Dinner</span>
                        </label>
                        <label class="meal-checkbox">
                        ...
                        <label class="meal-checkbox">
                            <input type="checkbox" id="visitorSnack" value="snack">
                            <span>🍪 Snack</span>
                        </label>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        async function refreshGroupStatus() {
            ...
            const statusCard = document.getElementById('groupStatusCard');
            const badgesContainer = document.getElementById('groupStatusBadges');
            ...
            if (badgesContainer) badgesContainer.innerHTML = '<span style="color: var(--text-muted);">Loading...</span>';

            try {
                const response = await fetch(`{{ route('bulk.groupStatus') }}?date=${date}&location=${encodeURIComponent(location)}`);
                const groups = await response.json();

                if (groups.error) {
                    badgesContainer.innerHTML = '<span style="color: var(--danger);">Error loading status</span>';
                    return;
                }
                ...
                groups.forEach(group => {
                    ...
                    switch (group.status) {
                        ...
                        case 'partial':
                            statusColor = '#28a745';
                            statusIcon = '✅';
                            statusText = `${group.uploaded_count}/${group.total_employees}`;
                            break;
                        case 'empty':
                            statusColor = '#6c757d';
                            statusIcon = '📭';
                ...
                badgesContainer.innerHTML = html || '<span style="color: var(--text-muted);">No groups found</span>';
            } catch (error) {
                console.error('Error fetching group status:', error);
                badgesContainer.innerHTML = '<span style="color: var(--danger);">Failed to load status</span>';
            }
        }

        // Auto-refresh group status when date or location changes
        document.addEventListener('DOMContentLoaded', function () {
            const dateInput = document.getElementById('date');
            const locationSelect = document.getElementById('location');

            if (dateInput) dateInput.addEventListener('change', refreshGroupStatus);
            if (locationSelect) locationSelect.addEventListener('change', refreshGroupStatus);

            // Initial load if both fields have values
            if (dateInput && dateInput.value && locationSelect && locationSelect.value) {
                refreshGroupStatus();
            }
        });
        ...
        function updateMealTotals() {
            ...
            document.getElementById('totalSnack').textContent = snackCount;
        ...
        // Handle dinner checkbox change - auto check/uncheck snack
        function handleDinnerChange(entryIndex) {
            const entryEl = document.getElementById(`entry-${entryIndex}`);
            if (!entryEl) return;

            const dinnerCheckbox = entryEl.querySelector('input[value="dinner"]');
            const snackCheckbox = entryEl.querySelector('input[value="snack"]');

            if (!snackCheckbox) return;

            if (dinnerCheckbox && dinnerCheckbox.checked) {
                // Dinner checked: auto-check snack and disable it
                snackCheckbox.checked = true;
                snackCheckbox.disabled = true;
            } else {
                // Dinner unchecked: enable snack but don't uncheck it
        ...
        // Auto-select proof if date and location match an existing proof
        function autoSelectProof() {
            const dateInput = document.getElementById('date');
            const locationSelect = document.getElementById('location');
            const proofSelect = document.getElementById('existing_proof');

            if (!proofSelect || !dateInput || !locationSelect) return;

            const selectedDate = dateInput.value;
            const selectedLocation = locationSelect.value;

            // Look for a matching proof
            let matchFound = false;
            for (let i = 0; i < proofSelect.options.length; i++) {
                const option = proofSelect.options[i];
                const proofDate = option.dataset.date;
                const proofLocation = option.dataset.location;

                if (proofDate === selectedDate && proofLocation === selectedLocation) {
            ...
            // If no match found, clear the selection (unless user uploaded a file)
            if (!matchFound) {
        ...
        function openVisitorModal() {
            ...
            document.getElementById('visitorSnack').checked = false;
            document.getElementById('visitorSnack').disabled = false;
